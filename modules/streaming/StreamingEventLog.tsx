  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-white/10 bg-black/25">
      ...
      <div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto px-2 py-2">
        {eventLog.length === 0 ? (
          <p className="px-2 py-8 text-center text-[11px] text-zinc-500">
          ...
          eventLog.map((evt) => (
            <article
              key={evt.id}
              className="rounded-lg border border-white/8 bg-zinc-900/60 px-2.5 py-2"
            >
              <div className="flex flex-wrap items-center gap-2">
                <span
                  className={`rounded-md border px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide ${typeBadgeClass(evt.type)}`}
                >
                  {TYPE_LABEL[evt.type]}
                </span>
                <time
              ...
              <p className="mt-1 text-[11px] leading-snug text-zinc-200">{evt.summary}</p>
              {evt.detail ? (
                <p className="mt-0.5 text-[10px] leading-snug text-zinc-500">{evt.detail}</p>
              ) : null}
            </article>
          ))
        )}
